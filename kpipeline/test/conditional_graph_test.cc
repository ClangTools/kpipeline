...
public:
    TestRouterNode() : Node(CreateConfig("Router", {"input"}, {"route_a", "route_b"})) {}
...
public:
    BranchProcessNode(const std::string& name, const std::string& control_signal)
        : Node(CreateConfig(name, {"branch_input"}, {"branch_output_" + name}, {control_signal})) {}

    void Execute(kpipeline::Workspace& ws) const override {
        std::string val = ws.Get<std::string>("branch_input");
...
public:
    MergeNode() : Node(CreateConfig("Merge", {"branch_output_BranchA", "branch_output_BranchB"}, {"final_output"})) {}
