...
public:
    explicit LoadImageNode(const Json::Value& config) : Node(config) {}
...
public:
    explicit ResizeImageNode(const Json::Value& config) : Node(config) {
        // 从配置中读取本节点特有的参数
...
public:
    explicit ExtractMetadataNode(const Json::Value& config) : Node(config) {}
...
public:
    explicit GenerateReportNode(const Json::Value& config) : Node(config) {}
